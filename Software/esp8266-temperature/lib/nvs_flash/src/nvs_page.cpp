...
{
    auto rc = spi_flash_write(getEntryAddress(mNextFreeEntry), (uint32_t*)&item, sizeof(item));
    if (rc != ESP_OK) {
        mState = PageState::INVALID;
        return rc;
    }
...
#endif //ESP_PLATFORM
    auto rc = spi_flash_write(getEntryAddress(mNextFreeEntry), (uint32_t*)buf, size);
#ifdef ESP_PLATFORM
    if (buf != data) {
        free((void*)buf);
    }
...
{
    auto rc = spi_flash_read(getEntryAddress(index), (uint32_t*)&dst, sizeof(dst));
    if (rc != ESP_OK) {
        return rc;
    }
    return ESP_OK;
